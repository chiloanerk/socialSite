<pre style="background-color: lightgrey"><?= var_dump($_SESSION) ?></pre>
...
<nav class="relative px-4 py-4 flex justify-between items-center bg-gray-800">
    ...
    <div class="lg:hidden">
        <button class="navbar-burger flex items-center text-gray-300 p-3">
    ...
    <ul class="hidden absolute top-1/2 left-1/2 transform -translate-y-1/2 -translate-x-1/2 lg:flex lg:mx-auto lg:flex lg:items-center lg:w-auto lg:space-x-6">
        <li><a href="/home" class="<?= urlIs('/') ? 'bg-gray-900 text-white' : 'text-gray-300' ?> hover:bg-gray-700 hover:text-white rounded-md px-3 py-2 text-sm font-medium">Home</a></li>
        <li class="text-gray-300">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" class="w-4 h-4 current-fill" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 5v0m0 7v0m0 7v0m0-13a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2z" />
            </svg>
        </li>
        <?php if ($_SESSION['user_token'] ?? false) : ?>
        <li>
                <a href="/inbox" class="<?= urlIs('/inbox') ? 'bg-gray-900 text-white' : 'text-gray-300' ?> hover:bg-gray-700 hover:text-white rounded-md px-3 py-2 text-sm font-medium">Inbox</a>
        </li>
        <?php endif; ?>
    </ul>
    <?php if ($_SESSION['user_token'] ?? false) : ?>
        ...
        <a class="hidden lg:inline-block py-2 px-6 bg-blue-500 hover:bg-blue-600 text-sm text-gray-300 font-bold rounded-xl transition duration-200" href="/logout">Sign out</a>
    <?php else : ?>
    ...
    <nav class="fixed top-0 left-0 bottom-0 flex flex-col w-5/6 max-w-sm py-6 px-6 bg-white border-r overflow-y-auto">
        ...
        <div>
            <ul>
                ...
                <li class="mb-1">
                    <a class="block p-4 text-sm font-semibold text-gray-400 hover:bg-blue-50 hover:text-blue-600 rounded" href="/notes">Notes</a>
                </li>
                <li class="mb-1">
        ...
        <div class="mt-auto">
            <div class="pt-6">
                <?php if ($_SESSION['user_token'] ?? false) : ?>
                    <a class="block px-4 py-3 mb-3 leading-loose text-xs text-center font-semibold leading-none bg-gray-50 hover:bg-gray-100 rounded-xl" href="/profile">Profile</a>
                    <a class="block px-4 py-3 mb-2 leading-loose text-xs text-center text-gray-300 font-semibold bg-blue-600 hover:bg-blue-700  rounded-xl" href="/logout">Sign Out</a>
                <?php else : ?>
